...
public class UnityChanInfomation : Player    // 캐릭터 정보를 담고 있는 스크립트
{
    ...
    private ParticleSystem m_rushEffect;
    ...
    void timerState()          // 타이머가 걸린 동작을 처리하는 함수
    {
        ...
        if (m_fCameraRotate > 0.0f)                                             // 회전 시간
        {
            ...
            transform.Rotate(Vector3.up, 800 * Time.deltaTime, Space.World);    // 회전
        }

        if (m_aniState.IsName("HoldAttack1") == true || m_aniState.IsName("HoldAttack2") == true)
        {
    ...
    void flash() // 대쉬공격 상태 처리
    {
        ...
        else if(m_aniTransition.IsName("Recovery -> Idle") == true)
        {
            initAnimatorInfo();
        }
    ...
    {
        if (m_aniTransition.IsName("AnyState -> Hide") == true)
        {
    ...
    public void hideEnd()
    {
        ...
        if (s_eventPlayerState != null)      // 델리게이트 체크
            s_eventPlayerState(6666);
    }

    public void leafAttack()
    {
        if (m_aniTransition.IsName("Roll -> LeafAttackChargeStart") == true)            // 만약 구르기에서 리프어택 전이가 일어났으면
        {
        ...
        if (m_aniState.IsName("LeafAttackChargeStart") == true)
        {
            ...
            else if (m_animator.GetBool("leafAttack")==true)
            {
        ...
        {
            transform.Translate(transform.forward * m_fLeafAttackTimer * 0.3f, Space.World);
        }
    ...
    private void OnTriggerEnter(Collider coll)                // 공격 충돌 처리
    {
        if (coll.gameObject.tag == "enemy")                   // 충돌 대상이 적 태그를 가지고 있으면
        {
            ...
            if (m_fCurAttackDamage < 0.0f)
                enemyScript.attacted(-m_fCurAttackDamage, CROWD_CONTROL.CRITICAL);         // 그 적은 내 현재 공격모션의 데미지를 부여함
            else
    ...
    public void levelUp()
    {
        ...
        ProfileUIManager.Instance.levelUp();
    }
}
