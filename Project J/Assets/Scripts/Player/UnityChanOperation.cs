...
public enum PLAYER_STATE
{
    ...
    CHARGE_SKILL_1,     // 차지스킬1 : 검기 날리기
    CHARGE_SKILL_2,     // 차지스킬2 : 검기 날리기
    HOLD_SKILL_1,       // 홀드스킬 : 난무
    SKILL_END,

    STEP = 8,   // 스텝 스킬 (빠르게 이동)
    ROLL = 9,   // 구르기 스킬 (다양한 스킬 캔슬 및 무적 부여)
    HIDE = 13,  // 하이딩 스킬
}
...
public class UnityChanOperation : Player   // 유니티짱 조작 스크립트
{
    ...
    {
        if (Input.GetKey(KeyCode.Keypad3))        // 좌우 회전
        {
    ...
    void rush()
    {
        ...
        {
            if (Input.GetKey(KeyCode.Keypad3) && Input.GetKey(KeyCode.W))
            {
    ...
    void baseAttack()
    {
        if (Input.GetKeyDown(KeyCode.Keypad4) == true)         // 마우스 왼쪽 키를 눌럿으면
        {
    ...
    void holdAttack()
    {
        ...
        {
            if (Input.GetKeyDown(KeyCode.Keypad5) == true)         // 마우스 왼쪽 키를 눌럿으면
            {
    ...
    public void assassination()                                      // 암살 스킬
    {
        ...
        else if (m_iStateLevel == (int)PLAYER_STATE.HIDE)
        {
            ...
            {
                m_animator.SetTrigger("cut");                                                            // 절단 트리거 발동
    ...
    void chargeAttack()
    {
        ...
        {
            if (Input.GetKeyDown(KeyCode.Keypad6) == true)         // 마우스 왼쪽 키를 눌럿으면
            {
    ...
    void leafAttack()
    {
        if (CharacterInfoManager.instance.coolTimeCheck((int)SKILL_TYPE.LEAF_ATTACK) == false) // 쿨타임이 존재하지 않을 시
        {
            ...
            {
                if (m_animator.GetBool("leafAttack") == true)
                {
